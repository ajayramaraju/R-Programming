...
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, shape = drv, color=drv))#Instead this aestethic works very well with the drive mode of the car because there are only three different type of drive mode
```
...
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy), color = "lightblue", size= 4, shape= 4)#The y (hwy) maps value available in the hwy column; viceversa the x maps to the displ column values. All the aesthetics are mapped manually.
```
...
ggplot(data = mpg)+
  geom_bar(mapping = aes(y=displ, fill=class), fill="red")
...
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) + 
  facet_wrap(~class, nrow = 2)#The formula simply divide the original chart in as many type of car classes are available in your dataset
```
...
ggplot(data = mpg)+
  geom_point(mapping=aes(y=cty, x=cyl, color=drv))+
...
ggplot(data = mpg) + 
  ...
  facet_grid(cyl ~ drv)#The first argument of facet_grid() is also a formula. This time the formula should contain two variable names separated by a ~
```
...
ggplot(data = mpg)+
  geom_point(mapping = aes(y=cty, x=cyl, alpha=drv))+
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  ...
  geom_smooth()# the color applies only to the points because it is specified in that geom. While the other aesthetics apply to both because they are specified in the global settings of the chart.
```
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  ...
  geom_smooth(data= filter(mpg, class!= "subcompact"), color= "pink")# just keep in mind that we are basically filtering only the subcompact cars to draw the line. This can be extremely powerful if you want to show how they perform compared to all the others.
...
ggplot(data = mpg, mapping = aes(x=displ, y=hwy))+
  ...
  geom_smooth(mapping = aes(color=as_factor(cyl)))
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  ...
  scale_color_colorblind()+
  theme_dark()#try classic, bw, minimal and linedraw as other nice alternatives
```
...
## Exploratory Data Analisis (EDA) in full circle [not in the exam]
