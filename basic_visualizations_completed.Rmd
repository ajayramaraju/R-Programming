...
```{r fig.width = 6}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, size = class, color=class))#Warning: Using size for a discrete variable is not advised. What happen if you assign size to cyl?
...
ggplot(data = mpg)+
  geom_bar(mapping = aes(x=manufacturer, alpha=class, fill=class))
...
```{r fig.width = 6, message = FALSE,warning=FALSE,echo=TRUE}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, shape = drv, color=drv))#What happened to the SUVs? ggplot2 will only use six shapes at a time. By default, additional groups will go unplotted when you use the shape aesthetic.
