...
filter(flights, month == 1)#the results do not replace the original dataset. To save it to future analysis assign it to a new variable named jan_flights. Create the variable jan_flights 
...
filter(flights, month==11 | month == 12)# look at the results. Does it make sense to have such filter?

#Example 4: find flights in November or December
view(filter(flights, month == 11 | month == 12))# Now this makes sense as you want to constraint the focus of your analysis to the last two months of the year. You might have an intuition that during the holiday season there is need for more flights/personnel 

# Example 5: find flights that weren’t delayed (on arrival or departure) by more than two hours 
filter(flights, dep_delay <= 120 | arr_delay <= 120)# Hours are presented in minutes in the dataset that's why 120. Again, you need to get to know your data before starting manipulations. If you don't remember column names check the colnames function (?colnames).
...
view(filter(flights, month == 1, dep_delay > 60 | arr_delay > 60))
...
```{r}
view(arrange(flights, month, day))
view(arrange(flights, desc(dep_delay))#desc allows to re-order by a column in descending order
